...
for t in [0.4, 0.5, 0.6, 0.7, 0.8, 0.9]:
	...
	results = {}
	for layers in layers_list:
		with h5py.File(f"./results/rqc_scaling_L{L}_t{t}_layers{layers}.hdf5", "r") as f:
			results[layers]  =  f["Vlist"][:]"""
	...
	for layers in layers_list:
		...
		err = np.abs(1-np.abs(np.trace(ansatz(results[layers], L, perms
			).conj().T @ scipy.linalg.expm(-1j * t * hamil)))/2**8)
		#print(err)
		errs.append(err)
