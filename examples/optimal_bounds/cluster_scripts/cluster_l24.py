...
for t in ts:
    ...
    V1 = scipy.linalg.expm(-1j*t*hloc/16)
    V2 = scipy.linalg.expm(-1j*t*hloc/8)
    Vlist_reduced_24 = [V1, V2, V1]*8
    perms_24 = [[[i for i in range(L)]] if i%2==0 else [[i for i in range(1, L)]+[0]] for i in range(len(Vlist_reduced_24))]
    ...
    Vlist, f_iter, err_iter = optimize(L, hamil, t, Vlist_reduced_24, perms_24, rS=rS, niter=niter, log=True)
    with h5py.File(f".results/t{t}_layers24.hdf5", "w") as f:
